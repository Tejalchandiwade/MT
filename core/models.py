...
class Generator(nn.Module):
    def __init__(self, config_G ,bblock):
        ...
        for i in range(self.num_blocks):
            if self.bblock=="SE_with_ResNet":
                cur_block = G_block_SE(num_of_channels[i], num_of_channels[i+1], self.norm_name, i==0)
            elif self.bblock=="self_attn_with_ResNet":
                cur_block = G_block_attention_layer(num_of_channels[i], num_of_channels[i+1], self.norm_name, i==0)
            elif self.bblock=="CBAM_with_ResNet":
                cur_block = G_block_CBAM(num_of_channels[i], num_of_channels[i+1], self.norm_name, i==0)
            elif self.bblock=="ResNet":
                cur_block = G_block(num_of_channels[i], num_of_channels[i+1], self.norm_name, i==0)
            else: raise ValueError('Unexpected input for generator block')
...
class G_block_SE(nn.Module):
    def __init__(self, in_channel, out_channel, norm_name, is_first):
        ...
        self.se= SELayer(out_channel,self.reduction)
        #self.avg_pool=nn.AdaptiveAvgPool2d(1)
        #self.fc = nn.Sequential(
            #nn.Linear(out_channel, out_channel // self.reduction, bias=False),
            #nn.ReLU(inplace=True),
            #nn.Linear(out_channel // self.reduction, out_channel, bias=False),
            #nn.Sigmoid())
...
class G_block_attention_layer(nn.Module):
    def __init__(self, in_channel, out_channel, norm_name, is_first):
        super(G_block_attention_layer, self).__init__()
        middle_channel = min(in_channel, out_channel)
        self.ups = nn.Upsample(scale_factor=2) if not is_first else torch.nn.Identity()
        self.activ = nn.LeakyReLU(0.2)
        self.conv1 = sp_norm(nn.Conv2d(in_channel,  middle_channel, 3, padding=1))
        ...
        self.norm2  = get_norm_by_name(norm_name, middle_channel)
        self.conv_sc = sp_norm(nn.Conv2d(in_channel, out_channel, (1, 1), bias=False))
        self.self_att = nn.MultiheadAttention(embed_dim=out_channel, num_heads=8)
...
class G_block_CBAM(nn.Module):
    def __init__(self, in_channel, out_channel, norm_name, is_first):
        super(G_block_CBAM, self).__init__()
        middle_channel = min(in_channel, out_channel)
        self.ups = nn.Upsample(scale_factor=2) if not is_first else torch.nn.Identity()
        self.activ = nn.LeakyReLU(0.2)
        self.conv1 = sp_norm(nn.Conv2d(in_channel,  middle_channel, 3, padding=1))
        ...
        self.channelAtt = ChannelAttention(out_channel)
